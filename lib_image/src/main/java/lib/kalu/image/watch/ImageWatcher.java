...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    public class DefaultLoadingUIProvider implements LoadingUIProvider {
        ...
        public View initialView(ViewGroup parent) {
            ...
            lib.kalu.image.watch.ProgressView progressView = new lib.kalu.image.watch.ProgressView(parent.getContext());
        ...
        public void start(View loadView) {
            ...
            ((lib.kalu.image.watch.ProgressView) loadView).start();
        }

        @Override
        public void stop(View loadView) {
            ((lib.kalu.image.watch.ProgressView) loadView).stop();
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                if (mPagerPositionOffsetPixels == 0) { // 正在查看一张图片，不处于翻页中。
                    if (mTouchMode != TOUCH_MODE_SCALE) {
                        ...
                        mFingersCenterX = 0;
                        mFingersCenterY = 0;
                        lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_TOUCH_SCALE);
                    }
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        if (mTouchMode == TOUCH_MODE_DOWN) {
            ...
            if (Math.abs(moveX) > mTouchSlop || Math.abs(moveY) > mTouchSlop) {
                final lib.kalu.image.watch.ViewState vsCurrent = lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_CURRENT);
                final lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
                ...
                if (vsDefault == null) {
                    ...
                    if (mTouchMode != TOUCH_MODE_EXIT) {
                        lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_EXIT);
                    }
                    ...
                    if (mTouchMode != TOUCH_MODE_DRAG) {
                        lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_DRAG);
                    }
    ...
    public boolean onSingleTapConfirmed() {
        ...
        lib.kalu.image.watch.ViewState vsCurrent = lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_CURRENT);
        lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
        if (vsDefault == null || (vsCurrent.scaleY <= vsDefault.scaleY && vsCurrent.scaleX <= vsDefault.scaleX)) {
            mExitRef = 0;
        } else {
            iSource.setTag(lib.kalu.image.watch.ViewState.STATE_EXIT, vsDefault);
            mExitRef = 1;
        }
        handleExitTouchResult();
        return true;
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if (iSource != null && mTouchMode != TOUCH_MODE_AUTO_FLING && mPagerPositionOffsetPixels == 0) {
            ...
            if (getCurrentPosition() <= 0 && velocityX > 0 && vsCurrent.translationX == (vsDefault.width * (vsCurrent.scaleX - 1) / 2)) {
                ...
                animFling(iSource, lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_TEMP).translationX(endTranslateX).translationY(endTranslateY), (long) (1000000 / maxVelocity));
                return true;
            }
        }
        return false;
    ...
    private void handleExitGesture(MotionEvent e2, MotionEvent e1) {
        ...
        lib.kalu.image.watch.ViewState vsExit = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_EXIT);
        lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
    ...
    private void handleDragGesture(MotionEvent e2, MotionEvent e1) {
        if (iSource == null) return;
        final lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
        final lib.kalu.image.watch.ViewState vsTouchDrag = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DRAG);
    ...
    private void handleScaleGesture(MotionEvent e2) {
        if (iSource == null) return;
        final lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
        final lib.kalu.image.watch.ViewState vsTouchScale = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_TOUCH_SCALE);
    ...
    private void handleDoubleTapTouchResult() {
        ...
        if (vsCurrent.scaleY <= vsDefault.scaleY && vsCurrent.scaleX <= vsDefault.scaleX) {
            ...
            animSourceViewStateTransform(iSource,
                    lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_TEMP).scaleX(expectedScale).scaleY(expectedScale));
        } else {
    ...
    private void handleScaleTouchResult() {
        ...
        lib.kalu.image.watch.ViewState vsTemp = lib.kalu.image.watch.ViewState.copy(vsDefault, lib.kalu.image.watch.ViewState.STATE_TEMP).scaleX(endScaleX).scaleY(endScaleY);
        ...
        iSource.setTag(lib.kalu.image.watch.ViewState.STATE_TEMP, vsTemp);
    ...
    private void handleDragTouchResult() {
        ...
        animSourceViewStateTransform(iSource,
                lib.kalu.image.watch.ViewState.write(iSource, lib.kalu.image.watch.ViewState.STATE_TEMP).translationX(endTranslateX).translationY(endTranslateY));
    ...
    private void handleExitTouchResult() {
        ...
        if (mExitRef > 0.75f) {
            ...
            lib.kalu.image.watch.ViewState vsOrigin = lib.kalu.image.watch.ViewState.read(iSource, lib.kalu.image.watch.ViewState.STATE_ORIGIN);
            if (vsOrigin != null) {
                if (vsOrigin.alpha == 0) {
    ...
    public void onPageSelected(int position) {
        ...
        if (lib.kalu.image.watch.ViewState.read(mLast, lib.kalu.image.watch.ViewState.STATE_DEFAULT) != null) {
            lib.kalu.image.watch.ViewState.restoreByAnim(mLast, lib.kalu.image.watch.ViewState.STATE_DEFAULT).create().start();
        }
        ImageView mNext = adapter.mImageSparseArray.get(position + 1);
        if (lib.kalu.image.watch.ViewState.read(mNext, lib.kalu.image.watch.ViewState.STATE_DEFAULT) != null) {
            lib.kalu.image.watch.ViewState.restoreByAnim(mNext, lib.kalu.image.watch.ViewState.STATE_DEFAULT).create().start();
        }
    }
    ...
    class ImagePagerAdapter extends PagerAdapter {
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos, boolean hasPlayBeginAnimation) {
            ...
            lib.kalu.image.watch.ViewState.write(imageView, lib.kalu.image.watch.ViewState.STATE_ORIGIN).alpha(0).scaleXBy(1.5f).scaleYBy(1.5f);
            ...
            if (originRef != null) {
                ...
                lib.kalu.image.watch.ViewState.write(imageView, lib.kalu.image.watch.ViewState.STATE_ORIGIN).width(originRef.getWidth()).height(originRef.getHeight());
                ...
                if (bmpMirror != null) {
                    ...
                    lib.kalu.image.watch.ViewState vsThumb = lib.kalu.image.watch.ViewState.write(imageView, lib.kalu.image.watch.ViewState.STATE_THUMB).width(bmpMirrorWidth).height(bmpMirrorHeight)
                    ...
                    if (isFindEnterImagePicture) {
                        ...
                        lib.kalu.image.watch.ViewState.restore(imageView, vsThumb.mTag);
                    }
                }
            }
            ...
            lib.kalu.image.watch.ViewState.clear(imageView, lib.kalu.image.watch.ViewState.STATE_DEFAULT);
            ...
            loader.load(imageView.getContext(), mUrlList.get(pos), new LoadCallback() {
                ...
                public void onResourceReady(Drawable resource) {
                    ...
                    lib.kalu.image.watch.ViewState vsDefault = lib.kalu.image.watch.ViewState.write(imageView, lib.kalu.image.watch.ViewState.STATE_DEFAULT).width(sourceDefaultWidth).height(sourceDefaultHeight)
                    ...
                    if (isPlayEnterAnimation) {
                        ...
                        lib.kalu.image.watch.ViewState.restore(imageView, vsDefault.mTag);
     ...
     */
    private void animSourceViewStateTransform(ImageView view, final lib.kalu.image.watch.ViewState vsResult) {
        if (view == null) return;
        if (animImageTransform != null) animImageTransform.cancel();

        animImageTransform = lib.kalu.image.watch.ViewState.restoreByAnim(view, vsResult.mTag).addListener(mAnimTransitionStateListener).create();

        if (animImageTransform != null) {
            if (vsResult.mTag == lib.kalu.image.watch.ViewState.STATE_ORIGIN) {
    ...
    private void animFling(ImageView view, final lib.kalu.image.watch.ViewState vsResult, long duration) {
        if (duration > 800) duration = 800;
        else if (duration < 100) duration = 100;
        if (animFling != null) animFling.cancel();
        animFling = lib.kalu.image.watch.ViewState.restoreByAnim(view, vsResult.mTag).addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationStart(Animator animation) {
                mTouchMode = TOUCH_MODE_AUTO_FLING;
            }
