 ...
 */
final class PhotoImageView extends AppCompatImageView {
...
//    private static int SCREEN_WIDTH = 0, SCREEN_HEIGHT = 0;
    ...
    private Bitmap decodeByte(byte[] data) {
        ...
        final boolean result = options.outWidth > options.outHeight;
        final int width = display.getWidth();
        final int height = display.getHeight();
        final int screenWidth = result ? height : width;
        final int screenHeight = result ? width : height;
    ...
    private final GestureDetector mSimpleOnGestureListener = new GestureDetector(getContext().getApplicationContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            // 双击图片缩放的最大或最小
            if (mState == PhotoImageView.State.NONE && !mIsTranslate && !mIsRotate && !mIsScale) {
                float targetZoom = (mNormalizedScale == 1) ? mMaxScale : 1;
                mScale = new PhotoImageView.Scale(targetZoom, e.getX(), e.getY(), true);
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            mFling = new PhotoImageView.Fling((int) velocityX, (int) velocityY);
            mFling.start();
